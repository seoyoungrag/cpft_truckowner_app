...
import { Dimensions, Text, TouchableOpacity, Modal, View, StyleSheet, KeyboardAvoidingView, Alert } from "react-native";
...
export default ({ refreshFn, loading, order, year, month }) => {
	...
	return (
		<OuterContainer>
			<Modal animationType="fade" hardwareAccelerated={true} transparent={true} statusBarTranslucent={true} visible={queryModalVisible}>
				<KeyboardAvoidingView behavior="padding" enabled style={styles.centeredView}>
					...
					<View style={{ flexDirection: "row", bottom: 0 }}>
						<ConfirmBtn
							onPress={() => {
								Alert.alert("문의완료!", "문의가 완료되었습니다.", [{ text: "네", onPress: () => {} }], { cancelable: false });
			...
			<Modal animationType="fade" hardwareAccelerated={true} transparent={true} statusBarTranslucent={true} visible={applyModalVisible}>
				<KeyboardAvoidingView behavior="padding" enabled style={styles.centeredView}>
					<View style={styles.modalView}>
						<View style={styles.modalInnerView}>
							<DataHeader>
					...
					<View style={{ flexDirection: "row", bottom: 0 }}>
						...
						<ConfirmBtn
							onPress={() => {
								Alert.alert("지원완료!", "지원이 완료되었습니다.", [{ text: "네", onPress: () => {} }], { cancelable: false });
								setApplyModalVisible(!applyModalVisible);
							}}
						>
			...
			{order.tmpKey != 1 && order.tmpKey != 2 && order.tmpKey != 3 ? (
				<DetailFooter>
					...
					>
						<Text style={{ fontSize: 16, color: "#3E50B4", fontWeight: "bold" }}>오더 거절</Text>
					</CancelBtn>
					<ConfirmBtn
						onPress={() => {
							Alert.alert(
