...
export default ({navigation, route}) => {
	...
	const deductionInfo = rq.useQuery(
		"getDeductionList",
		async () => {
			return await axios.post("https://blueapi.teamfresh.co.kr/v2/trans/getDeductionList", {
				matchingCode: matchingCode,
				deliveryDate: targetMonth,
			});
		},
	...
	const penaltyInfo = rq.useQuery(
		"getPenaltyList",
		async () => {
			return await axios.post("https://blueapi.teamfresh.co.kr/v2/trans/getPenaltyList", {
				matchingCode: matchingCode,
				deliveryDate: targetMonth,
			});
		},
	...
	return (
		...
		>
				...
				<View style={{padding: 20}}>
					...
					<View style={{backgroundColor: "white", marginTop: 10, padding: 17}}>
						<View style={{flexDirection: "row", justifyContent: "flex-start"}}>
							<Text style={{fontWeight: "bold", fontSize: 18, color: "black"}}>{Calc.getMonthStr(targetMonth) || "-"}월</Text>
						</View>
            ...
            <Text>{Calc.regexWON(dataInfo?.data?.data?.data?.addedAllwnc) || "-"}원</Text>
						...
						<View style={{flexDirection: "row", justifyContent: "space-between", marginTop: 10, marginBottom: 10}}>
							<Text>발생 운행경비</Text>
							<Text>{Calc.regexWON(dataInfo?.data?.data?.data?.transExtraPay) || "-"}원</Text>
						</View>
