...
export default ({navigation, route}) => {
  ...
  const paymentInfo = rq.useQuery("getPaymentList", async () => {
  ...
  const deductionInfo = rq.useQuery("getDeductionList", async () => {
  ...
  const penaltyInfo = rq.useQuery("getPenaltyList", async () => {
          ...
          <Collapse onToggle={(bool) => setStatus(prevStatus => ({...prevStatus, isOpen1: bool}))}>
            <CollapseHeader>
              <View style={{flexDirection: "row", justifyContent: "space-between", paddingTop: 20, borderTopWidth: 1, borderTopColor: "#efefef", marginBottom: 20}}>
                <Text style={{fontWeight: "bold", color: "black"}}>
                  <Image source={require("../../assets/img/icon_info.png")}/>&nbsp;차감 상세 내역
                </Text>
                {isOpen1 ? 
            ...
            <CollapseBody>
              ...
              <View style={{marginBottom: 20}}>
                ...
                && deductionInfo.data.list.map((data, index, array) => <DeductionDetail key={index} data={data} bool={index+1 === array.length? true : false} array={array} />)}
              </View>
            </CollapseBody>
          </Collapse>
