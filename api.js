...
const makeRequestCpft = (path, config) => axios.get(`https://blueapi.teamfresh.co.kr${path}`, config);
...
const makePostRequestCpft = (path, config) => axios.post(`https://blueapi.teamfresh.co.kr${path}`, config);
...
export const orderApi = {
  ...
  truckOwnerOrders: (status, userSeq) => getAnythingCpft(`/v1/mobile/order/status/${status}/truckOwner/${userSeq}`)
};

export const barobillApi = {
  getTaxInvoicePopUpURLUsingPOST: (mgtKey) => postAnythingCpft(`/v1/barobill/getTaxInvoicePopUpURL?mgtKey=${mgtKey}`)
};
